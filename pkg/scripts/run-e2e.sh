...
build_and_push_mock_plugin() {
    docker buildx build \
        ...
        --progress=plain \
        -f /Users/kaijun/Go/src/kms/Dockerfile \
        /Users/kaijun/Go/src/kms;
...
create_cluster_and_run_test() {
    ...
    # kubetest2 kind -v 5 \
    # --build \
    # --kube-root /Users/kaijun/Go/src/kubernetes \
    # --up \
    # --rundir-in-artifacts \
    # --config /Users/kaijun/Go/src/kms/pkg/scripts/kind.yaml \
    # --cluster-name "${cluster_name}" ${TEST_ARGS}
    kind create cluster --config ./kind.yaml --retain
...
main(){
    ...
    # mkdir -p _output/dockerized/bin/linux/arm64;
    # for binary in kubectl e2e.test ginkgo; do
    #     cp -f _output/local/go/bin/${binary} _output/dockerized/bin/linux/arm64/${binary};
    # done;
